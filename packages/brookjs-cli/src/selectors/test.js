...
import R from 'ramda';
import { lAppDir, lCommandName, lCommandArgs, lEnvCwd } from '../lenses';
...
export const getMochaCommand = state =>
    `mocha ${getTestReporter(state)} ${getTestUi(state)} ${getTestRequires(state)} --colors ${getTestFilesGlob(state)}`;
